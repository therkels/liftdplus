...
/** Pick the best identifier the route can read: slug -> display_id -> id */
function getPostPath(post: {
  slug?: string | null;
  display_id?: string | null;
  id?: number | string;
}) {
  const key = post?.slug ?? post?.display_id ?? post?.id;
  return key ? `/post/${key}` : "/discover";
}
...
const Card: React.FC<CardProps> = ({
  ...
  const cardData = {
    image: post.cover_image_url || "/liftd-og-default.png",
    title: post.title || "Untitled",
    secondaryTitle: (post as any).secondary_title ?? null,
    authorName: (post as any).author_name ?? (post as any).contributor_name ?? (post as any).author ?? "LIFTD+",
    authorPhoto: (post as any).author_photo ?? null,
    likes: likeCount,
    tags: [
    ...
    readTime: readTime || `${(post as any).read_time_minutes ?? 5} minute read`,
  };
  ...
  return (
    ...
    >
      <div
        className={`relative ${compact ? "h-32 md:h-40" : "h-48 md:h-56"} w-full`}
      >
        <Image
          src={cardData.image}
          alt={cardData.title}
      ...
      >
        <div>
          <h2
            className={`${
              compact ? "text-sm md:text-base leading-tight" : "text-xl md:text-2xl leading-tight"
