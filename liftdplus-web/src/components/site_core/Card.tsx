...
interface CardProps {
  ...
  onClick?: () => void;           // parent can handle navigation / modal
  compact?: boolean;
  readTime?: string;
...
const Card: React.FC<CardProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        <Image src={cardData.image} alt={cardData.title} fill className="object-cover" />
      </div>
      ...
      >
        <div>
          ...
          {cardData.secondaryTitle && (
            <div className="mt-1 flex items-center gap-1">
              ...
              <span className="text-xs md:text-sm text-subtext flex-shrink-0">More</span>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between pt-2">
          <div className="flex items-center space-x-2">
            ...
            <span
              className={`${compact ? "text-[10px] md:text-xs" : "text-xs md:text-sm"} font-medium text-gray-700 leading-tight`}
            >
              {cardData.authorName}
            </span>
          </div>
