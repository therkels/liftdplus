...
const STORAGE_KEY = "liftd_legacy_q1";
const MAX_SELECT = 3;

export default function LegacyOnboardingQ1Page() {
  const router = useRouter();
  const [selected, setSelected] = useState<Set<string>>(new Set());

  useEffect(() => {
    sendGAEvent("event", "onboarding_legacy_q1_viewed", {});
  }, []);

  useEffect(() => {
    trackEvent("onboarding_step_viewed", { step: "legacy_q1" });
  }, []);

  const toggle = (option: string) => {
    trackEvent("onboarding_answer_selected", { step: "legacy_q1", answer: option });
    setSelected((prev) => {
      if (prev.has(option)) {
  ...
  const handleContinue = () => {
    ...
    const selectedTopics = Array.from(selected);
    sendGAEvent("event", "onboarding_legacy_q1_completed", { topics: selectedTopics });
    if (typeof window !== "undefined") {
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ topics: selectedTopics }));
    }
    router.push("/onboarding/legacy/q2");
  };

  const count = selected.size;
  const canContinue = count === MAX_SELECT;
        ...
        >
          ...
          <div className="text-center mt-2 mb-6">
            ...
            <p className="text-sm text-white/40 leading-relaxed">
              Choose 3 topics you&apos;d like to explore first. We&apos;ll personalize your feed
          ...
          <p
            ...
            style={{
              color: count === MAX_SELECT ? "#ccff33" : "rgba(255,255,255,0.3)",
            }}
          >
            {count} of 3 selected
          </p>
