...
const STORAGE_KEY = "liftd_legacy_q3";

export default function LegacyOnboardingQ3Page() {
  const router = useRouter();
  const [selected, setSelected] = useState<string | null>(null);

  useEffect(() => {
    sendGAEvent("event", "onboarding_legacy_q3_viewed", {});
  }, []);

  useEffect(() => {
    trackEvent("onboarding_step_viewed", { step: "legacy_q3" });
  }, []);

  const select = (option: string) => {
    setSelected(option);
    trackEvent("onboarding_answer_selected", { step: "legacy_q3", answer: option });
  };

  const handleContinue = () => {
    if (!selected) return;
    sendGAEvent("event", "onboarding_legacy_q3_completed", { purchaseBehavior: selected });
    if (typeof window !== "undefined") {
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ purchaseBehavior: selected }));
    }
    router.push("/onboarding/legacy/q4");
  };
        ...
        >
          ...
          <div className="w-full flex items-center justify-between mb-6">
            <div
              role="button"
              tabIndex={0}
              onClick={() => router.push("/onboarding/legacy/q2")}
              onKeyDown={(e) => e.key === "Enter" && router.push("/onboarding/legacy/q2")}
