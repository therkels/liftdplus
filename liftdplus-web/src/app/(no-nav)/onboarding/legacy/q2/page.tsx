...
export default function LegacyOnboardingQ2Page() {
  ...
  useEffect(() => {
    sendGAEvent("event", "onboarding_legacy_q2_viewed", {});
  }, []);

  useEffect(() => {
    trackEvent("onboarding_step_viewed", { step: "legacy_q2" });
  }, []);

  const select = (option: string) => {
    setSelected(option);
    trackEvent("onboarding_answer_selected", { step: "legacy_q2", answer: option });
  };
        ...
        >
          ...
          <div className="w-full flex items-center justify-between mb-6">
            <div
              role="button"
              tabIndex={0}
              onClick={() => router.push("/onboarding/legacy")}
              onKeyDown={(e) => e.key === "Enter" && router.push("/onboarding/legacy")}
